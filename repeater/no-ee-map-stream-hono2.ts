...
class ConnectionManager {
  ...
  removeHandlerFromAllChannels(handler: Handler) {
    const channels = this.getChannelsByHandler(handler);
    channels?.forEach((channel) => {
      this.removeHandlerFromChannel(channel, handler);
    });
  }
  ...
  const s = new ReadableStream({
    start(controller) {
      ...
      handleAbort(c, stream, () => {
        cm.removeHandlerFromAllChannels(handler);
...
app.get("/subscribe/:channel", (c) => {
  ...
  const channelToSubscribeTo = c.req.param("channel");

  handlers?.forEach((handler) => {
    cm.addHandlerToChannel(channelToSubscribeTo, ssid!, handler);
  });
  ...
  cm.addChanneltoSsid(channelToSubscribeTo, ssid!);

  return c.json({ ok: true });
});

app.get("/unsubscribe/:channel", (c) => {
  const ssid = getCookie(c, "ssid");
  const channelToUnsubscribeFrom = c.req.param("channel");
